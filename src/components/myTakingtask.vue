  ...
  export default {
    methods: {
      ...
      gettasks() {
        setTimeout(
          () => {
            ...
            axios.post("/myTakingTask", this.MyTaskDTO, config)
              .then((res) => {
                console.log(res);
      ...
      searchtasks() {
        setTimeout(
          () => {
            ...
            axios.post("/myTakingTask/search", this.MyTaskDTO, config)
              .then((res) => {
                console.log(res);
                this.tableData = res.data.records
                this.total = res.data.total
      ...
      goToTaskDetail(taskId) {
        ...
        // 跳转到对应的详情页
        this.$router.push({ path: path, query: { taskId: taskId } });
      },
      handleSearch() {
    ...
    data() {
      return {
        tableData: [],
        MyTaskDTO: {
          state: "incomplete", // 状态筛选条件
          sortRule: "finish_time", // 排序属性
          page: 1,
          isDesc: true, // 是否降序
          keyword: ""
        },
